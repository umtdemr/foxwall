...
def test_follow_request_create(valid_user, valid_user2):
    follow_request_obj = FollowRequest.create_follow_request(
        creator_id=valid_user.id, target_user_id=valid_user2.id)
    ...
    try:
        FollowRequest.create_follow_request(creator_id=valid_user.id,
...
def test_follow_create(valid_user, valid_user2):
    follow_obj = Follow.create_follow(user_id=valid_user.id,
    ...
    try:
        Follow.create_follow(user_id=valid_user.id,
                             followed_user_id=valid_user2.id)
    except IntegrityError:
        assert 1
